      ...
      <div className='col-lg-7'>
       ...
       <tbody>
        {
          cartArray.map((product,index)=>(
            <tr key={index}>
              ...
              <td>{product.price}</td>
              <td><button onClick={()=>dispatch(removeFromCart(product.id))} className='btn'><i class="fa-solid text-danger fa-trash"> </i> </button></td>
            </tr>
          ))
        }
       ...
       <div className='col-lg-3'>
        <div className='border mt-3 rounded shadow p-2 w-100'>
         ...
         <button className='btn btn-success mt-5 rounded' onClick={handleCart}> Checkout</button>
          </div>
        </div>
       </div>
       </table>
